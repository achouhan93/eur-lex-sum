...
def pages_extraction(provided_url: str):
    """
    Function extracts the number of pages that needs to be considered for extracting the Celex Numbers

    Args:
        provided_url (string): URL of the Domain specific Legal Acts, for example: Energy, Agriculture, Taxation,
            and others Legal Acts: https://eur-lex.europa.eu/browse/directories/legislation.html
    ...
    if len(page_number_indexes) == 0:
        ...
        last_page_number = int((re.search(r'page=(\d+)', last_page_number_url, re.IGNORECASE)).group(1)) + 1
...
def get_celex(pages: int, provided_url: str) -> List:
    """
    Function extracts all the Celex Number of the documents from the considered URL

    Args:
        pages (integer): The number of pages that need to be considered for extracting the Celex Numbers
        provided_url (string): URL of the Domain specific Legal Acts, for example: Energy, Agriculture, Taxation,
            and other Legal Acts: https://eur-lex.europa.eu/browse/directories/legislation.html
            Energy Legal Acts: https://eur-lex.europa.eu/search.html?type=named&name=browse-by:legislation-in-force&CC_1_CODED=12&displayProfile=allRelAllConsDocProfile
    ...
    for i in range(1, pages):
        # Introduce an artificial wait to avoid timing out
        sleep(1)
        # A separate URL is created for each page of the legal act domain
        url = urllib.request.urlopen(provided_url + '&page=' + str(i)).read()

        # Scraping the Page using the BeautifulSoup Library
        soup = BeautifulSoup(url, 'lxml')

        # Fetching celex numbers by parsing html tags hierarchy and checking for text 'CELEX number'
        try:
...
def celex_main(provided_url: str) -> List[str]:
    """
...
def get_document_summary(lang: str, celex_id: str):
    ...
    if 'No legislative summaries' in document_request.text:
        ...
        if list_of_documents is None:
            ...
            summary_url = 'https://eur-lex.europa.eu/' + list_of_documents['href'][
...
def get_file_by_id(lang: str, celex_id: str) -> Dict:
    """
    Functionality: Extract the content present in the Celex document

    Signature of the function:
     Input:
          lang (string): Document content language
          celex_id (string): Celex ID of the document whose content needs to be extracted
    ...
    # Tracking dictionary which type of document (HTML / PDF / NONE) in respective language
    track_dict = {
        'celex_id': celex_id
    }
    ...
    try:
        ...
        if 'The requested document does not exist.' in HTML_content:
            ...
            pdf_document_path = os.path.join(pdf_directory, f"{celex_id}_{lang}.pdf")
        ...
        result_dict['documentContent'] = document_content

    except:
        track_dict[lang] = "NONE"
        result_dict['documentContent'] = "NA"
...
def elastic_search_mapping() -> Dict:
    """
...
def elastic_search_create(es_index: opensearchpy.client, index_name: str, es_mapping: Dict):
    """
...
def elastic_search_insert(es_index: opensearchpy.client, index_name: str, celex_information: Dict):
    """
    ...
    Signature of the function:
     Input:
          esIndex: ElasticSearch or OpenSearch connection
          indexName: Name of the index that needs to be created
          celexInformation: Document content of a single Celex document (in all languages)

     Output:
          Insert the information in the ElasticSearch or OpenSearch Index keeping the celex ID as a unique ID (_id)
    """
    doc = {
        "bulgarian":
            {
...
def elastic_search_existing_check(es_index: opensearchpy.client, index_name: str, celex_list: List[str]) -> List[str]:
    """
    ...
    Signature of the function:
     ...
     Output:
          List of all the celex number that are not present in the ElasticSearch or OpenSearch index
    """
    non_existing = []
...
if __name__ == '__main__':
    ...
    # Iterate through all years with available documents in reverse order
    for year in range(2022, 1950, -1):
        print('################')
        print(f'Year => {year}')
        # Iterate all available domains of the Legal Acts (Agriculture, Energy, etc.)
